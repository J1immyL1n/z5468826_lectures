...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    csv_file_name = f"{modified_tic}_prc.csv"
...
def monthly_return_cal(prc):
    ...
    # Resample to get the last business day of each month and calculate monthly returns
    monthly_returns = prc.resample('M').last().pct_change()

    # Count the number of observations per month
    monthly_counts = prc.resample('M').count()

    # Filter out months with fewer than 18 entries
    monthly_returns = monthly_returns[monthly_counts >= 18]

    # Drop the first NaN value which is a result of the pct_change calculation
    monthly_returns = monthly_returns.dropna()

    # Convert DateTimeIndex to PeriodIndex of format 'Year-Month'
    monthly_returns.index = monthly_returns.index.to_period('M')

    # Rename the index
    monthly_returns.index.rename('Year_Month', inplace=True)
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        ...
        monthly_returns[ticker.lower()] = monthly_return_cal(price_data)

    daily_returns_df = pd.DataFrame(daily_returns).rename_axis('Date', axis='index')
    monthly_returns_df = pd.DataFrame(monthly_returns).rename_axis('Year_Month', axis='index')


    return_dict = {"Daily":daily_returns_df, "Monthly":monthly_returns_df}
